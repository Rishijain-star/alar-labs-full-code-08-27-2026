...
const isRefreshableTokenError = (error) => {
    ...
    if (status === 401) {
        ...
        // "No user" on protected routes often means missing/expired access token in the client
        // while the server session cookie may still be valid — refresh can recover.
        return true;
    }
...
axiosInstance.interceptors.response.use(
    ...
    async (error) => {
        ...
        const isInvalidSessionMessage = /invalid\s+session|expired\s+session|session\s+invalid|session\s+expired/i.test(message);
            ...
            try {
                ...
                if (result.data?.success === false) {
                    ...
                    const invalidSession =
                        /invalid|expired session/i.test(msg) ||
