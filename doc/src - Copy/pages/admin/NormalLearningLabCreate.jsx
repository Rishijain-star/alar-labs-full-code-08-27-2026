...
const createEmptyBlock = (type) => {
  ...
  switch (type) {
    ...
    case "alert":
      return { ...base, title: "", message: "", alertType: "info" };
    case "externalLink":
...
const BLOCK_EDITORS = {
  ...
  alert: function AlertBlockEditor({ block, onChange }) {
    const update = (updates) => onChange({ ...block, ...updates });
    return (
      <div className="space-y-4">
        ...
        <div>
          ...
          <Input value={block.title} onChange={e => update({ title: e.target.value })} placeholder="Alert title" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Alert Type</label>
          <Select value={block.alertType} onValueChange={(val) => update({ alertType: val })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Textarea value={block.message} onChange={e => update({ message: e.target.value })} placeholder="Alert message..." rows={4} />
        </div>
      </div>
    );
  },
...
export default function SkillBuilderPro() {
  ...
  const duplicateBlock = (id) => {
    ...
    const newBlock = { ...blockToDuplicate, id: uid() };
    updateLesson(currentLesson.id, { blocks: [...currentLesson.blocks, newBlock] });
    setDialogBlockId(newBlock.id);
    setBlockDialogOpen(true);
  };
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-slate-900">{lab.title || "Add Lab"}</h1>
          </div>
