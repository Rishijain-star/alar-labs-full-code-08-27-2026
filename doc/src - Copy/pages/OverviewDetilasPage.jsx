...
export default function OverviewDetailsPage() {
  ...
  const isEnrolled = !!lab?.isPurchased;
  const outlineModules = lab ? normalizeOutlineModules(lab.modules, { isEnrolled }) : [];
  const ctaLabel = lab ? getEnrollCtaLabel({ isEnrolled, isFree: lab.isFree }) : "Enroll Now";
  ...
  const loadLab = useCallback(async () => {
    ...
    try {
      ...
      const mapped = mapLabViewPayload(raw);
      if (mapped) {
        setLab(mapped);
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f8]" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div className="bg-gradient-to-br from-[#0f1f3d] via-[#1a3560] to-[#1e4080]">
        <div className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-[7fr_3fr] gap-8 items-start">
            ...
            <div>
              ...
              {isEnrolled ? (
                ...
                <Button
                  type="button"
                  size="lg"
                  className="bg-blue-600 hover:bg-blue-700 text-white font-bold gap-2 h-12 rounded-lg shadow-xl cursor-pointer"
                  onClick={handleEnroll}
                >
                  {ctaLabel}
                </Button>
          ...
          {/* Hero Right (30%) — just the thumbnail/video with nice border/rounded */}
                              ...
                              {les.tasks.map((task) => {
                                ...
                                return (
                                  <div key={task.id} className="flex items-center gap-3 px-5 pl-14 py-2.5">
                                    ...
                                    <Lock className={cn("w-3 h-3 flex-shrink-0", isEnrolled ? "text-green-400" : "text-slate-300")} />
                                  </div>
                                );
                              })}
                            </div>
