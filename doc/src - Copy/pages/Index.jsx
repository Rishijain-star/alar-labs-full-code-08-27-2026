...
function mapPublicLabRow(l) {
  ...
  return {
    ...
    level,
    rating:
      typeof l.rating === "number" && Number.isFinite(l.rating) ? l.rating : undefined,
    enrolledCount:
...
const productSectionsBase = [
  {
    ...
    icon: ClipboardCheck,
    href: "/programs/technology_readiness_assessment",
    color: "primary",
  },
  {
    id: "training",
    ...
    icon: Users,
    href: "/programs/expert_led_training",
    color: "secondary",
  },
  {
    id: "certification",
    ...
    description: "Role-based pathways and certification-aligned learning tracks.",
    icon: Award,
    href: "/programs/tech_career_pathways",
    color: "primary",
  },
];
...
export default function Index() {
    ...
    () =>
      productSectionsBase.map((ps) => {
        ...
        return {
          ...
          description: cms?.subtitle || ps.description,
        };
      }),
    [cmsSectionByKey]
  );
  ...
  const skillLabsSectionCopy = useMemo(() => {
    const cms = cmsSectionByKey.skill_builder_labs;
    return {
      eyebrow: "Practice Makes Perfect",
      title: cms?.title || "Skill Builder Labs",
      subtitle: cms?.subtitle || "Hands-on practice in real cloud environments",
    };
  }, [cmsSectionByKey]);

  const digitalProgramsSectionCopy = useMemo(() => {
    const cms = cmsSectionByKey.digital_skill_program;
    return {
      eyebrow: "Master The Tools",
      title: cms?.title || "Digital Skills Programs",
      subtitle: cms?.subtitle || "On-demand digital training for real-world skill building",
    };
  }, [cmsSectionByKey]);
  ...
  const homeCourses = useMemo(() => {
    const list = normalizeCoursesPayload(coursesData);
    return list.map((c) => ({
      id: c.id || c._id,
      slug: c.slug,
      title: c.title,
    ...
    <div>
      ...
      {!bannersLoading && heroSlides.length > 0 && (
        <section className="relative isolate w-full overflow-hidden bg-slate-800" style={{ minHeight: "95vh" }}>
          ...
          <div className="relative z-10 flex flex-col" style={{ minHeight: "95vh" }}>

            {/* Content wrapper */}
            <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 flex items-center flex-1 py-24">
              {/* Hero text content */}
              <div
              ...
              >
                ...
                <Badge
                  variant="outline"
                  className="mb-8 rounded-full border-white/30 bg-white/10 px-4 py-1.5 text-xs font-semibold tracking-wide text-white shadow-none hover:bg-white/20"
                >
                ...
                <h1 className="mb-8 font-extrabold tracking-tight">
                  <span className="block text-2xl sm:text-3xl md:text-4xl lg:text-[2.2rem] xl:text-5xl leading-[1.15] text-white">
                    {heroSlides[currentHero].title}
                  </span>
                </h1>

                <p className="mb-10 text-sm leading-relaxed text-white/90 sm:text-base max-w-md">
                ...
                <div className="flex flex-row items-center gap-3 flex-wrap sm:flex-nowrap">
                  <Link to={heroSlides[currentHero].button_link || "/labs"} className="shrink-0">
                    <Button
                      size="sm"
                      className="h-10 gap-2 rounded-lg px-5 font-semibold shadow-md whitespace-nowrap text-sm bg-primary hover:bg-primary/90"
                    >
                      <Monitor className="h-4 w-4 shrink-0" /> {heroSlides[currentHero].button_title || "Get Started"}
                    </Button>
                  </Link>
                </div>
              </div>
            ...
            {productSections.map((section, index) => {
              ...
              return (
                <Link key={section.id} to={section.href}>
                  <Card
                    className="h-full card-hover border border-slate-300 rounded-xl cursor-pointer group transition-all duration-300 hover:border-green-500 hover:shadow-lg hover:-translate-y-1"
      ...
      <section ref={statsRef} className="py-16 bg-muted">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-semibold mb-2 text-foreground">Our Achievements</h2>
            <p className="text-muted-foreground">Trusted by learners and organizations worldwide</p>
          </div>
      ...
      {/* Partners - White */}
      <section className="py-28 border-b border-border overflow-hidden bg-background">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-foreground mb-3">Our Partners &amp; Technology Platforms</h2>
          </div>
        </div>
        <div className="relative w-full overflow-hidden">
          <div className="flex gap-24 animate-marquee whitespace-nowrap items-center">
            {[...partners, ...partners, ...partners].map((partner, index) => (
              <div
                key={index}
                className="flex flex-col items-center justify-center min-w-[180px] gap-4"
              >
                <img
                  src={partner.logo}
                  alt={partner.name}
      ...
      <section className="py-16 bg-muted" id="labs-section">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <h2 className="text-2xl font-bold text-foreground mb-1">{skillLabsSectionCopy.title}</h2>
              <p className="text-muted-foreground text-sm">{skillLabsSectionCopy.subtitle}</p>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" onClick={() => scroll(labsScrollRef, "left")} className="rounded-full">
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => scroll(labsScrollRef, "right")} className="rounded-full">
      ...
      <section className="py-16 bg-background" id="courses-section">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="text-xs font-semibold text-secondary uppercase tracking-wide mb-1">
              ...
              <h2 className="text-2xl font-bold text-foreground mb-1">{digitalProgramsSectionCopy.title}</h2>
              <p className="text-muted-foreground text-sm">{digitalProgramsSectionCopy.subtitle}</p>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon" onClick={() => scroll(coursesScrollRef, "left")} className="rounded-full">
                <ChevronLeft className="w-4 h-4" />
              </Button>
              <Button variant="outline" size="icon" onClick={() => scroll(coursesScrollRef, "right")} className="rounded-full">
                <ChevronRight className="w-4 h-4" />
              </Button>
              <Link to="/courses">
                <Button variant="ghost" size="sm" className="gap-1 ml-2">
