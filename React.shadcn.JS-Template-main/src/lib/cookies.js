...
/** Secure cookies are ignored on plain HTTP — omit the flag unless we're on HTTPS. */
    ...
    */
    ...
    setToken(token, expiresInDays = 7) {
        if (token) {
            ...
            document.cookie = `${TOKEN_KEY}=${token}; expires=${expires.toUTCString()}; path=/; SameSite=Strict${secureCookieSuffix()}`;
            try {
    ...
    setRefreshToken(refreshToken, expiresInDays = 30) {
        if (refreshToken) {
            const expires = new Date();
            expires.setDate(expires.getDate() + expiresInDays);
            document.cookie = `${REFRESH_TOKEN_KEY}=${refreshToken}; expires=${expires.toUTCString()}; path=/; SameSite=Strict${secureCookieSuffix()}`;
        }
    },
    ...
    setSessionId(sessionId, expiresInDays = 30) {
        if (sessionId) {
            const expires = new Date();
            expires.setDate(expires.getDate() + expiresInDays);
            document.cookie = `${SESSION_ID_KEY}=${sessionId}; expires=${expires.toUTCString()}; path=/; SameSite=Strict${secureCookieSuffix()}`;
        }
    },
    ...
    setUser(userData, expiresInDays = 7) {
        if (userData) {
            ...
            const expires = new Date();
            expires.setDate(expires.getDate() + expiresInDays);
            document.cookie = `${USER_KEY}=${encodeURIComponent(userString)}; expires=${expires.toUTCString()}; path=/; SameSite=Strict${secureCookieSuffix()}`;
        }
    },
