...
/** Normalize role labels/ids for comparison ("Super Admin" → "super_admin"). */
...
/** Known aliases between menu role slugs and DB role names. */
const ROLE_EQUIVALENTS = {
    super_admin: ["super_admin", "superadmin"],
    admin: ["admin", "administrator"],
    instructor: ["instructor", "course_instructor"],
    creator: ["creator", "content_manager", "course_instructor"],
...
export function rolesMatch(userRole, requiredRole) {
    ...
    const r = normalizeRoleKey(requiredRole);
    if (!r) return true;
    if (!u) return false;
    if (u === r) return true;

    for (const aliases of Object.values(ROLE_EQUIVALENTS)) {
        const set = new Set(aliases.map(normalizeRoleKey));
        if (set.has(u) && set.has(r)) return true;
    }
    return false;
}

/** True when any user role matches any required role slug. */
export function hasAnyRequiredRole(requiredRoles = [], userRolesList = []) {
    if (!requiredRoles.length) return true;
    if (!userRolesList.length) return false;
    return requiredRoles.some((req) =>
        userRolesList.some((ur) => rolesMatch(ur, req))
    );
}

export function collectUserRoleKeys(user, userRolesFromStore = []) {
    const keys = [];
    if (user?.role?.name) keys.push(user.role.name);
    if (user?.role?.id) keys.push(user.role.id);
    if (user?.role && typeof user.role === "string") keys.push(user.role);

    for (const r of userRolesFromStore) {
        if (typeof r === "string") keys.push(r);
        else {
            if (r?.name) keys.push(r.name);
            if (r?.id) keys.push(r.id);
        }
    }
    return keys;
}
...
import { expandImpliedPermissions } from "@/lib/permissionBundles";
...
class PermissionStore {
    ...
    getAllUserPermissions() {
        ...
        // Combine, deduplicate, and expand bundle implications
        const allPermissions = expandImpliedPermissions(
    ...
    hasRole(roleId) {
        ...
        return this.userRoles.some((r) => {
            ...
            return candidates.some((c) => rolesMatch(c, roleId));
        });
    }
...
/** Super Admin / Admin bypass — mirrors backend rbacService.checkUserHasPermission */
function hasFullAccessRole() {
    permissionStore.initialize();

    const roleKeys = [
        ...collectUserRoleKeys(null, permissionStore.getUserRoles()),
    ];
    ...
    try {
        ...
        if (userStr) {
            ...
            roleKeys.push(...collectUserRoleKeys(user, []));
        }
    } catch {
        /* ignore */
    }

    const fullAccessRoles = ["super_admin", "superadmin", "admin", "administrator"];
    return roleKeys.some((key) =>
        fullAccessRoles.some((required) => rolesMatch(key, required))
    );
}

// Patch store methods so all permission checks honour full-access roles
const _hasPermission = PermissionStore.prototype.hasPermission;
PermissionStore.prototype.hasPermission = function hasPermissionWithBypass(permissionId) {
    if (hasFullAccessRole()) return true;
    return _hasPermission.call(this, permissionId);
};

const _hasAnyPermission = PermissionStore.prototype.hasAnyPermission;
PermissionStore.prototype.hasAnyPermission = function hasAnyPermissionWithBypass(permissionIds) {
    if (hasFullAccessRole()) return true;
    return _hasAnyPermission.call(this, permissionIds);
};

const _hasAllPermissions = PermissionStore.prototype.hasAllPermissions;
PermissionStore.prototype.hasAllPermissions = function hasAllPermissionsWithBypass(permissionIds) {
    if (hasFullAccessRole()) return true;
    return _hasAllPermissions.call(this, permissionIds);
};

const _canAccessRoute = PermissionStore.prototype.canAccessRoute;
PermissionStore.prototype.canAccessRoute = function canAccessRouteWithBypass(route) {
    if (hasFullAccessRole()) return true;
    return _canAccessRoute.call(this, route);
};
...
export function isSuperAdminUser() {
    ...
    try {
        const userStr = localStorage.getItem("user");
        if (userStr) {
            roleKeys.push(...collectUserRoleKeys(JSON.parse(userStr), []));
        }
    } catch {
        /* ignore */
    }
    return roleKeys.some((key) =>
        ["super_admin", "superadmin"].some((r) => rolesMatch(key, r))
    );
}

export function hasFullAccessUser() {
...
export const saveUserPermissionsToCache = (userPermissions) => {
    try {
        ...
        const expanded = expandImpliedPermissions(
            permissionsArray.map((p) => (typeof p === "string" ? p : (p?.id || p?.name))).filter(Boolean)
        );
            
        localStorage.setItem(STORAGE_KEYS.USER_PERMISSIONS, JSON.stringify(expanded));
        localStorage.setItem(STORAGE_KEYS.LAST_UPDATED, Date.now().toString());
