...
const AdminUsers = () => {
  ...
  const canViewUsers = isSuperAdmin() || hasAnyPermission(["view_users"]);
      ...
      <div className="p-6 max-w-4xl mx-auto text-center py-16">
        <Card className="border-red-200 bg-red-50/50 p-8 shadow-sm">
          ...
          <p className="text-sm text-red-600 max-w-md mx-auto">
            You do not have permission to view all users. Please contact your system administrator to grant access.
          </p>
        </Card>
      </div>
    );
  ...
  const totalPages = usersData?.data?.pagination?.totalPages
    ?? usersData?.data?.pagination?.total_pages
    ?? usersData?.data?.totalPages
    ?? Math.max(1, Math.ceil(totalUsers / limit));
  ...
  const handleDeleteUser = async (userId) => {
    if (!(await confirmDelete("this user"))) return;
    try {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!(await confirmDelete(`${selectedUsers.length} user(s)`))) return;
    try {
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {hasAnyPermission(["view_users"]) && (
                          <DropdownMenuItem
                            onClick={() =>
                              navigate(`/app/users/${user.user_id}/details`, {
                                state: {
                                  user: {
                                    id: user.user_id,
                                    email: user.email,
