...
const BLOCK_TYPES = [
  ...
  {
    id: "pdf",
    label: "PDF / Document Block",
    icon: File,
    color: "text-red-600",
    bg: "bg-red-50",
    border: "border-red-200",
    description: "Add PDFs and documents"
  },
  {
    id: "alert",
    label: "Alert Block",
    ...
    description: "Add info/warning alerts"
  },
  {
    id: "externalLink",
    label: "External Link Block",
    icon: ExternalLink,
    color: "text-teal-600",
    bg: "bg-teal-50",
...
const createEmptyBlock = (type) => {
  ...
  switch (type) {
    ...
    case "pdf":
      return { ...base, title: "", description: "", files: [{ id: uid(), name: "", url: "" }] };
    case "alert":
      return { ...base, title: "", message: "", alertType: "info", accentColor: "" };
    case "externalLink":
      return { ...base, title: "", url: "", description: "", buttonText: "Visit Link" };
    case "divider":
      return { ...base, title: "", style: "line" };
    case "download":
      return {
        ...base,
        title: "",
...
const BLOCK_EDITORS = {
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div><label className="block text-sm font-semibold text-slate-700 mb-1"> Description</label><Textarea value={block.description} onChange={e => update({ description: e.target.value })} placeholder="Brief description of this image block..." rows={2} /></div>
  ...
  pdf: function PDFBlockEditor({ block, onChange }) {
    const update = (updates) => onChange({ ...block, ...updates });
    const updateFile = (fileId, updates) => {
      const newFiles = (block.files || []).map(f => f.id === fileId ? { ...f, ...updates } : f);
      update({ files: newFiles });
    };
    const addFile = () => update({ files: [...(block.files || []), { id: uid(), name: "", url: "", file: null }] });
    const removeFile = (fileId) => update({ files: (block.files || []).filter(f => f.id !== fileId) });
    ...
    const handleFileChange = (fileId, e) => {
      ...
      if (file) {
        const url = URL.createObjectURL(file);
        updateFile(fileId, { file, url, name: file.name });
      }
    };
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-red-200">
          ...
          <h3 className="font-bold text-red-900">PDF / Document Block</h3>
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Title</label>
          <Input value={block.title} onChange={e => update({ title: e.target.value })} placeholder="Document title" />
        </div>
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-semibold text-slate-700">Documents</label>
            <Button variant="secondary" size="sm" onClick={addFile}><Plus className="w-4 h-4 mr-1" />Add Document</Button>
          </div>
          {(block.files || []).map((file, idx) => (
            <Card key={file.id} className="mb-3">
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <div className="flex-1 space-y-3">
                    <Input value={file.name} onChange={e => updateFile(file.id, { name: e.target.value })} placeholder={`Document ${idx + 1} Name`} />
                    <div className="space-y-2">
                      <label className="block text-sm font-semibold text-slate-700 mb-1">Upload Document (PDF)</label>
                      <Input type="file" accept=".pdf,.doc,.docx,.txt" onChange={(e) => handleFileChange(file.id, e)} />
                      {file.url && (
                        <div className="mt-2">
                          <div className="flex items-center gap-2 p-2 bg-slate-50 rounded border">
                            <File className="w-4 h-4 text-slate-600" />
                            <span className="text-sm truncate flex-1">{file.name || "Document"}</span>
                    ...
                    <div>
                      ...
                      <Input value={file.url} onChange={e => updateFile(file.id, { url: e.target.value })} placeholder="https://example.com/document.pdf" />
                    </div>
                  </div>
                  {(block.files || []).length > 1 && (
                    <Button variant="ghost" size="icon" onClick={() => removeFile(file.id)} className="text-red-500"><Trash2 className="w-4 h-4" /></Button>
                  )}
                </div>
              </CardContent>
            </Card>
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Description</label>
          <Textarea value={block.description} onChange={e => update({ description: e.target.value })} placeholder="Document description" rows={3} />
        </div>
      </div>
    );
  },
  externalLink: function ExternalLinkBlockEditor({ block, onChange }) {
    const update = (updates) => onChange({ ...block, ...updates });
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-teal-200">
          <ExternalLink className="w-5 h-5 text-teal-600" />
          <h3 className="font-bold text-teal-900">External Link Block</h3>
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Title</label>
          <Input value={block.title} onChange={e => update({ title: e.target.value })} placeholder="Link title" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">URL</label>
          <Input value={block.url} onChange={e => update({ url: e.target.value })} placeholder="https://example.com" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Button Text</label>
          <Input value={block.buttonText} onChange={e => update({ buttonText: e.target.value })} placeholder="Visit Link" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Description</label>
          <Textarea value={block.description} onChange={e => update({ description: e.target.value })} placeholder="Brief description..." rows={3} />
        </div>
      </div>
    );
  },
  divider: function DividerBlockEditor({ block, onChange }) {
    const update = (updates) => onChange({ ...block, ...updates });
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
          <Divide className="w-5 h-5 text-gray-600" />
          <h3 className="font-bold text-gray-900">Divider Block</h3>
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Section Heading (Optional)</label>
          <Input value={block.title} onChange={e => update({ title: e.target.value })} placeholder="Section heading" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">Style</label>
          <Select value={block.style} onValueChange={(val) => update({ style: val })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="line">Simple Line</SelectItem>
              <SelectItem value="decorated">Decorated Line</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div><label className="block text-sm font-semibold text-slate-700 mb-1"> Description</label><Textarea value={block.shortDescription} onChange={e => update({ shortDescription: e.target.value })} placeholder="Brief description of these resources..." rows={2} /></div>
...
export default function SkillBuilderPro() {
  ...
  const labDraftKey = getEditorDraftKey("normal-lab-editor", editSlug || "new");
    ...
    const loadLab = async () => {
      ...
      try {
        ...
        if (labKind === "skill_builder") {
          navigate(`/app/labs/skill-builder-lab-edit/${editSlug}`, { replace: true });
          return;
        }
        ...
        if (!cancelled) {
          ...
          setLab((prev) => ({ ...prev, ...mappedLab, lab_kind: "hands_on" }));
  ...
  const deleteLessonFrom = (moduleId, lessonId) => {
    ...
    if (errors.length === 0) {
      try {
        ...
        if (result?.success || result?.data?.lab) {
          ...
          showSuccess(
            publish
              ? (isEditMode ? "Lab published successfully!" : "Lab published successfully!")
              : (isEditMode ? "Lab saved as draft!" : "Lab saved as draft!")
          );
          navigate("/app/labs/mine");
        } else {
          ...
          <div className="bg-white border-b border-slate-200 sticky top-0 z-40">
            <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-slate-900">{lab.title || (isEditMode ? "Edit Lab" : "Add Lab")}</h1>
                  ...
                  <CardContent className="space-y-6">
                    <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <QuillRichEditor
                          editorKey={editLabId || editSlug || "new-lab"}
                      ...
                      >
                        {publishStatus === "published" ? (
                          <>
                            ...
                            {isSubmitting ? "Publishing…" : (isEditMode ? "Publish Lab" : "Publish Lab")}
                          </>
                        ) : (
                          <>
