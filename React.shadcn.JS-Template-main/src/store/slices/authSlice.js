...
import { markTokenExpiry, clearTokenExpiry } from "@/lib/tokenExpiryStorage";
...
const persistAuthSuccess = (state, payload) => {
    ...
    markTokenExpiry(payload.expires_in ?? payload.expiresIn ?? 300);
};
    ...
    extraReducers: (builder) => {
        builder
            .addMatcher(
                ...
                (state, { payload }) => {
                    ...
                    const normalized = {
                        ...
                        expires_in: payload.data?.expires_in ?? payload.data?.expiresIn,
                    };
            ...
            .addMatcher(
                ...
                (state, { payload }) => {
                    const { user, accessToken, sessionId, permissions, roles, expires_in, expiresIn } = payload.data;
                    persistAuthSuccess(state, { user, accessToken, sessionId, permissions, roles, expires_in, expiresIn });
                    state.mfaAuth = null;
                }
            )
