  ...
  endpoints: (builder) => ({
    ...
    getCourses: builder.query({
      ...
      providesTags: (result, error, arg) => [
        { type: "Course", id: `LIST-${arg?.scope || "all"}-${arg?.search || ""}-${arg?.page || 1}` },
        "Course",
      ],
    }),
    ...
    // ── PUBLIC: GET /courses/slug/:slug/overview ────────────────────────────────────
    getCourseOverview: builder.query({
      query: ({ slug, params = {} }) => ({
        url: `/courses/slug/${encodeURIComponent(slug)}/overview`,
        method: "GET",
        params,
        meta: { withCredentials: true, showToast: false },
      }),
      providesTags: (_, __, { slug }) => [{ type: "Course", id: `OVERVIEW_${slug}` }],
      keepUnusedDataFor: 300,
    }),

    // ── PUBLIC: GET /courses/slug/:slug/view (full content for workspace) ──────────
    getCourseSlugView: builder.query({
      query: ({ slug, params = {} }) => ({
        url: `/courses/slug/${encodeURIComponent(slug)}/view`,
        method: "GET",
        params,
        meta: { withCredentials: true, showToast: false },
      }),
      providesTags: (_, __, { slug }) => [{ type: "Course", id: `VIEW_${slug}` }],
