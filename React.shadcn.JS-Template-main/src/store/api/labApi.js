  ...
  endpoints: (builder) => ({
    ...
    getLabs: builder.query({
      ...
      providesTags: (result, error, arg) => [
        { type: "Lab", id: `LIST-${arg?.scope || "all"}-${arg?.approval || "all"}-${arg?.page || 1}` },
        "Lab",
      ],
    }),
    ...
    // ── Lab overview page (public detail with enrollment status) ────────────
    getLabOverview: builder.query({
      query: ({ slug, params = {} }) => ({
        url: `/labs/slug/${encodeURIComponent(slug)}/overview`,
        method: "GET",
        params,
        meta: { withCredentials: true, showToast: false },
      }),
      providesTags: (_, __, { slug }) => [{ type: "Lab", id: `OVERVIEW_${slug}` }],
      keepUnusedDataFor: 300,
    }),

    // ── Lab view (full content for workspace page) ───────────────────────────
    getLabView: builder.query({
      query: ({ slug, params = {} }) => ({
        url: `/labs/slug/${encodeURIComponent(slug)}/view`,
        method: "GET",
        params,
        meta: { withCredentials: true, showToast: false },
      }),
      providesTags: (_, __, { slug }) => [{ type: "Lab", id: `VIEW_${slug}` }],
