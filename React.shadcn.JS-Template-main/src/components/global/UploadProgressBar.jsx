...
import { cancelActiveUpload } from "@/lib/uploadAbortRegistry";
...
export function UploadProgressBar() {
  ...
  const statusText = isError ? null
    : isDone ? (label?.includes("course") || label?.includes("Saving") ? "Course saved ✓" : "Upload complete ✓")
      : label?.includes("Saving") ? "Saving course…"
        : pct < 5 ? "Starting upload…"
          : pct < 95 ? "Uploading, please wait…"
            : "Finishing up…";

  const itemText = itemsTotal > 1
    ? isDone
      ? `All ${itemsTotal} files uploaded`
      : `File ${Math.min((itemsUploaded || 0) + 1, itemsTotal)} of ${itemsTotal}`
    : null;
        ...
        <div style={{
          ...
          <div style={{ padding: "14px 16px", position: "relative" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              {!isError && !isDone && (
                <button type="button" onClick={handleCancelUpload} style={{
                  flexShrink: 0, width: 24, height: 24, borderRadius: "50%",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  border: "none", background: "transparent", cursor: "pointer",
                  color: "hsl(var(--muted-foreground))",
